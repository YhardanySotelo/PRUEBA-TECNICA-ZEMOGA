CargarLocalStorage();
CargarVotos(); //da persistencia a los votos
...
function CargarLocalStorage(){
  ...
  var local  = localStorage.getItem("datosvotos");
  if(local == null){ // si el local storage esta vacio lo carga
      localStorage.setItem('datosvotos', JSON.stringify(datosvotos));
  }
  else{ //si no está vacío le da persistencia
    var json = JSON.parse(local);
     localStorage.setItem('datosvotos', JSON.stringify(json));
  }
};

//funcion que controla el estado de los votones de votación e invoca a la función de votación
...
function Votar(indice,tipo){
    ...
    //Actualizar el estado general de votación
   VisualizarEstadoGral(indice,votospositivos,votosnegativos);

}

//Actualiza el porcentaje de los votos
function VisualizarVotos(indice, positivos, negativos){
      ...
      if(total == 0){ // Si no hay votos se muestran las barras de porcentaje 50% y 50%
        total = 1;
        porc_positivos = 50;
        porc_negativos = 50;
      }
...
//función que controla el estilo de los botones de votación
...
//Función que controla el cambio de visualziación lista o grid
...
//función que da persistencia a los votos incluso despues de actualizar la página
function CargarVotos(){
  var local = localStorage.getItem("datosvotos");
  if(local != null){
    var json = JSON.parse(local);
    for(var i=0; i< json.datos.length; i++){
        var votospositivos = json.datos[i].votos.positivo;
        var votosnegativos = json.datos[i].votos.negativo;
        VisualizarVotos(i,votospositivos,votosnegativos);
        VisualizarEstadoGral(i,votospositivos,votosnegativos);
    } 
  }
}

//función que actualiza los botones de estado general de votación
function VisualizarEstadoGral(indice, votospositivos, votosnegativos){
